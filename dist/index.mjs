...
const it = (i) => (() => {
  ...
  let t;
  const [r, o] = A(), s = lt(() => {
    let a = Math.floor(r() / 60 / 60 / 24).toString().padStart(2, "0"), n = Math.floor(r() / 60 / 60 % 24).toString().padStart(2, "0"), c = Math.floor(r() / 60 % 60).toString().padStart(2, "0"), f = Math.floor(r() % 60).toString().padStart(2, "0");
    return a != "00" ? `${a}:${n}:${c}:${f}` : `${n}:${c}:${f}`;
  }), l = () => {
    const a = Date.now(), n = (i?.end - a) / 1e3;
    o(n), t = requestAnimationFrame(l);
  };
  return ft(() => {
...
function Nt(i) {
  let t = null, r = null;
  const [o, s] = A(), [l, a] = A(!1), [n, c] = A(!1), [f, x] = A(), [d, m] = A(), b = lt(() => o() != null && d() != null && d()?.length == (o()?.digits || 3)), T = () => {
    b() && (c(!0), t(d()).then((h) => rt(() => {
      a(!1), m(null);
    })).catch((h) => {
      console.error(h), x("verify failed"), a(!1);
    }).finally(() => c(!1)));
  };
  return mt(() => {
    i?.ref({
    ...
    get children() {
      ...
      return P.style.setProperty("display", "flex"), P.style.setProperty("justifyContent", "start"), P.style.setProperty("alignItems", "center"), P.style.setProperty("gap", "0.5em"), E(P, w(pt, {
        ...
        var R = v.outter, k = `${l() ? "visible" : "hidden"}`, S = v.container, u = `${l() ? "0% 0%" : "0% 50%"}`, q = `${l() ? "100" : "0"}`, M = v.content, U = v.head, Y = n(), Z = v.main, H = v.captchabox, J = v.foot, K = v.input, Q = o()?.digits || 3, X = n(), j = v.btn, tt = !b(), et = v.backdrop, I = `${l() ? "100" : "0"}`;
...
class Wt {
  ...
  request(t, r) {
    return new Promise(async (o, s) => {
      ...
      const a = ot(t), [n, c] = await this.m(this.process, a, "a aocaptcha request").catch((f) => s(f));
      if (c?.Messages?.[0]) {
        const {
          Data: f,
          Tags: x
        ...
        console.log("AoCaptcha Request Id : ", n), this.latest_request = m, o(m);
      } else
  ...
  verify(t, r) {
    return new Promise(async (o, s) => {
      const l = Date.now();
      this.wallet = r || this.wallet, this.process || s("missed the captcha process id"), t || s("missed request data."), t.paths || s("missed captcha data."), (!t.timestamp || t?.timestamp + t?.duration < l) && s("the captcha is expired."), !this.wallet && !r && s("missed wallet"), W.show(t, async (a) => {
        try {
          const n = ot({
            ...
            "Pushed-For": t?.id,
            "App-Name": "AoCaptcha"
          }), [c, f] = await this.m(this.process, n, a).catch((m) => s(m)), {
            Messages: x,
            Output: d
          } = f;
          if (x?.[0]) {
